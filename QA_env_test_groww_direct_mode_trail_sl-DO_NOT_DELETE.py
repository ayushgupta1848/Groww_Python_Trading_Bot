...
CONFIG = {
    ...
    "TRAIL_START_PROFIT": 1.50,  # Start trailing after this profit per unit (in points) #NEWCHANGE
    ...
    "user_confirmation_needed": True,   # or False
}
...
def find_option_by_premium_parallel(option_type, min_premium, max_premium,
                                    lots=1, funds_buffer=0.9):
    """
    Pure premium-based option selection.
    NO momentum, NO direction bias.
    """
    ...
    if not candidates:
        print(f"⚠️ No {option_type} options in premium range")
    ...
    if not affordable:
        print(f"⚠️ No affordable {option_type}")
        return None, None, None

    # Pick closest to mid premium
...
def detect_option_type_parallel(index, expiry, min_p, max_p, lots, funds_buffer=0.9):
    print(f"🔍 Detecting CE & PE purely on premium logic")

    ce_inst, ce_ltp, ce_lot = find_option_by_premium_parallel(
        "CE", min_p, max_p, lots, funds_buffer
    )

    pe_inst, pe_ltp, pe_lot = find_option_by_premium_parallel(
        "PE", min_p, max_p, lots, funds_buffer
    )

    if ce_inst and not pe_inst:
        return "CE", ce_inst, ce_ltp, ce_lot
    if pe_inst and not ce_inst:
        return "PE", pe_inst, pe_ltp, pe_lot
    if not ce_inst and not pe_inst:
        return None

    user_confirmation_needed = CONFIG.get("user_confirmation_needed", False)
    ...
    if user_confirmation_needed:
        ...
        if user_input.lower() in ("c", "ce" , "1"):

            return "CE", ce_inst, ce_ltp, ce_lot

        elif user_input.lower() in ("p", "pe" , "2"):
            return "PE", pe_inst, pe_ltp, pe_lot
        ...
        # If both exist → choose cheaper (less risk)
        if ce_ltp <= pe_ltp:
            return "CE", ce_inst, ce_ltp, ce_lot
        else:
            return "PE", pe_inst, pe_ltp, pe_lot
...
def auto_mode_runner():
    ...
    while True:
        ...
        if not opt_result:
            print("❌ Could not determine CE/PE. Retrying in 3 minutes...")
            send_telegram("❌ Could not determine CE/PE. Retrying in 3 minutes...")
            time.sleep(180)
            continue
        ...
        symbol = (
            ...
            or instrument.get("internal_trading_symbol")
        )
        ...
        print(
            ...
            f"LTP={ltp} | lot_size={lot_size} | groww_symbol={groww_symbol}"
        )
