...
| `GET /api/release_notes` | — | full `release_notes.json` (`{current_version, releases:[{version,date,title,sections:[{heading,items[]}],notes:[{id,text,ts}]}]}`); seeds the file from `_DEFAULT_RELEASE_NOTES` (v1.0.0) on first read |
