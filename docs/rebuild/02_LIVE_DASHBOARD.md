...
| `_pulse_sampler_loop()` | `sleep(2)` market hours (`_PULSE_SAMPLE_SEC`), `sleep(60)` closed | Premium Pulse: per index (NIFTY, SENSEX) resolves ATM CE/PE contracts (nearest expiry, strike step 50/100, re-resolves when spot drifts > 0.75×step from tracked ATM), backfills today's series from **5-min option candles** (`{EXCH}-{INDEX}-{DDMonYY}-{strike}-{CE|PE}`, 0.3 s pacing), then one batched FNO LTP call for all 4 symbols per tick → appends to `_pulse_state[index].series` (spot_series capped 15000); **every tick** runs `_pulse_record_decision()` → Final Call span history (see `/api/premium_pulse_decision`); every `_PULSE_VERDICT_SEC=300` computes `_pulse_compute()` and appends a verdict row (persisted to `.premium_pulse_verdicts.json`, reloaded on start if same date; Final Call history is in-memory only) |
...
Locks: `_lock` (snapshot), `_bot_lock`, `_trade_lock`, `_trail_ltp_lock`, `_idx_lock`, `_ltp_result_lock`, `_oi_history_lock`, `_vix_history_lock`, `_ptai_analysis_lock`, `_ptai_ai_lock`, `_ai_lock`, `_pulse_lock`, `_scalp_lock`, `_oi_ai_lock`, `_mb_ai_lock`, `_qs_lock`, `_pai_lock`, `_proc_lock`, `_ai_session_lock`.
...
| `/api/premium_pulse` | `index` ∈ NIFTY/SENSEX (def NIFTY, invalid → NIFTY) | `_pulse_compute(index)` → `{ok, index, ts, atm, expiry, ce_sym, pe_sym, spot, ce_ltp, pe_ltp, straddle, market_open, windows:{day,60m,15m,5m each {ce,pe,strad:{first,last,chg,pct,vel,eff,range_pct,n}, spot_pct, cls:FLAT|SLOW|STEADY|FAST}}, behavior, emoji, tone, action, reasons:[], confidence:LOW|MEDIUM|HIGH, traps:[{t,spike_pct,gave_back_pct}], decay:{active,text}, chart:{t,ce,pe} (1-min resampled), verdicts:[≤80 newest-first]}`. `cls` thresholds per window from `_PULSE_THRESH` (|Δ%|: 5m 1.5/4/8 · 15m 2.5/6/12 · 60m 4/10/20 · day 6/15/30); trap = ≥5 % run-up within 15 min giving back ≥60 % in the next 15 min (min premium ₹5); decay_1h = CE & PE both ≤ −3 % in 1h with |spot| < 0.35 % · decay_day = both ≤ −8 %; day `spot_pct` from `_idx_ohlc` open. Verdict side selection is **signed, not abs** — the named side is the RISING (buyable) one; a crashing PE must never be called the momentum side. Classes: activity `c5/c15` from max-abs move, momentum `c5up/c15up` from the rising side only. Priority: PREMIUM DECAY (decay & c5up ≤ SLOW) → TRAP/WHIPSAW (≥2 traps & eff<0.4) → FAST MOMENTUM/FAST BUT CHOPPY (c5up/c15up FAST, rising side's 15m eff ≥/< 0.55) → STEADY TREND (c15up STEADY, or c5up STEADY & c15up ≠ FLAT; eff ≥ 0.55, up15 > 0) → **ONE-SIDE CRUSH ({dn}↓)** (activity STEADY+/15m but nothing rising — one side dumped, other not paying yet) → SLIGHT MOVEMENT → FLAT; each carries a concrete action. Payload also carries `decision` + `decision_history` (see next row) |
| `/api/premium_pulse_chart` | `index` ∈ NIFTY/SENSEX (def NIFTY) | Chart-only feed polled every **3 s**: `{ok, ts, chart:{t,ce,pe} (1-min resampled via _pulse_chart_series), ce_ltp, pe_ltp}`; `{ok:false}` until ≥2 samples |
| `/api/premium_pulse_decision` | `index` ∈ NIFTY/SENSEX (def NIFTY) | Lightweight **Final Call + momo** feed, polled every 3 s: `{ok, index, ts, market_open, decision:{key,emoji,tone,label,side,why,since,secs}, history:[{from,to,secs,ticks,key,emoji,tone,label,side,why}] newest-first}`. Computed by `_pulse_decision_from(windows,traps,decay)` each sampler tick: active side = 5m pct×0.6 + 15m pct×0.4 (CE vs PE); priority STAY_OUT (decay & 5m FLAT/SLOW) → DONT_CHASE (trap in last 25 min & 5m spike ≥4%) → SCALP (5m FAST up, eff15 ≥0.5) → RIDE (15m STEADY+ up, eff15 ≥0.55, 5m > −1.5%) → SCALP_ONLY (≥2 traps & eff15 <0.4) → WATCH (5m SLOW up both windows) → WAIT. History = span-grouped (consecutive identical labels merge; `_pulse_record_decision` debounces: a **different** call must hold 2 consecutive ticks (~4 s) via `_dec_pending` before opening a new span), trimmed to `_PULSE_DECISION_KEEP=3600` s, survives ATM rolls, in-memory only (lost on restart). Response also carries `momo` (`_pulse_momo`): `{ce_ltp, pe_ltp, ce5, pe5, ce15, pe15, ce_on, pe_on}` where `*_on` = side RISING ≥1.5 % in 5m or ≥2.5 % in 15m — drives the blinking CE/PE cards (3 s poll, shared with the Final Call fetch) |
| `/api/regime_history` | — | non-blocking: serves `_regime_hist_cache`, kicks `_build_regime_history_async()` when stale and adds `building:true` (page re-polls every 5s). `build_regime_history()` → `{ts,generated,counts,{days:[{date,day,open,high,low,close,range_pts,range_pct,move_pts,move_pct,eff,gap_pct,vix,vix_chg_pct,label,key,partial}]}}`; 30-min in-memory cache (`_REGIME_HIST_TTL=1800`) |
...
| `/api/momentum/config` | any subset of the `_live_cast` map | merge-writes `momentum_config_override.json`; casts: `validate_orders,choppiness_enabled,consec_sl_brake,HARD_SL_ATR_BASED,min_score_filter,velocity_filter,place_target_order,HARD_SL_FIXED`→bool; `atr_source,_vix_config_note`→str; `min_premium,max_premium,velocity_pct,consistency_pct,TRAIL_START_PROFIT,HARD_SL_POINTS,HARD_SL_ATR_MULTIPLIER,HARD_SL_FIXED_POINTS`→float; `lots,atm_range,scan_seconds,poll_seconds,cooldown_sec,no_signal_wait_sec`→int. → `{"ok":true}` |
| `GET /api/autofit` | — | `{current, history[], interval_sec:180, next_in_sec, thresholds, cooloff_samples:5}` — the AUTO-FIT regime light for the Momentum Auto Bot. `current`/`history[]` items: `{t, ts, state:GREEN|AMBER|RED|CLOSED, raw_state, cooling, headline, action, triggers[], notes[], spot, atr, target_pts, components{}}`; each component `{kind:tape|setup|damage, label, value, unit, level:0|1|2, caps_at?, desc}` |
| `GET /api/release_notes` | — | full `release_notes.json` (`{current_version, releases:[{version,date,title,sections:[{heading,items[]}],notes:[{id,text,ts}]}]}`); seeds the file from `_DEFAULT_RELEASE_NOTES` on first read. On every later read `_merge_seed_releases()` adds any release shipped in a newer build that the file has never seen, sorts releases by semantic version desc and moves `current_version` up to `APP_VERSION` — user notes on existing releases are never touched, and the merge is idempotent |
| `POST /api/release_notes` | `{action:"add_note", version, text}` / `{action:"delete_note", version, id}` / `{action:"add_release", version, title}` | `_release_notes_action` — validates (non-empty text ≤2000 chars, known version, `N.N.N` version format, no duplicates), writes atomically via `.tmp` + `os.replace` under `_rn_lock`, returns `{"ok":true,"data":<doc>}` or `{"ok":false,"error":…}`. `add_release` prepends the release and moves `current_version` |
| `GET /api/momentum/config` | — | returns the raw contents of `momentum_config_override.json` (`{}` if missing/unreadable); used by `mbHydrateTiming()` to repopulate TARGET PTS / COOLDOWN S / NO-SIG S / exit mode on page load so a dashboard reload does not clobber a running bot |
...
pnl_today, margin, orders, mkt_idx, clean_ohlc,
...
| `.autofit_history.json` | `_autofit_loop()` every 180 s (atomic `.tmp` + `os.replace`) | `{date, current, history[]}` — the day's AUTO-FIT samples; reloaded on restart when `date` is today, reset on a new day. Gitignored (runtime state) |
| `release_notes.json` | seeded by `_load_release_notes()`; written by `_save_release_notes()` on every UI note/release action | version history rendered at the top of the Dashboard tab (`.rn-card`) with the current version echoed in the header chip `#hdr-ver`. Gitignored — it is runtime state, the v1.0.0 seed lives in `_DEFAULT_RELEASE_NOTES` |
| `momentum_config_override.json` | `_bot_start("momentum", config)` (whitelist) and `/api/momentum/config` (merge) | whitelist in `_bot_start`: `trade_mode,index,expiry,lots,exit_mode,min_premium,max_premium,atm_range,validate_orders,scan_seconds,poll_seconds,choppiness_enabled,consec_sl_brake,consec_sl_pause_min,HARD_SL_ATR_BASED,HARD_SL_ATR_MULTIPLIER,atr_source,min_score_filter,velocity_filter,TRAIL_START_PROFIT,cooldown_sec,no_signal_wait_sec,HARD_SL_POINTS,place_target_order,HARD_SL_FIXED,HARD_SL_FIXED_POINTS` |
...
| `.premium_pulse_verdicts.json` | `_pulse_save_verdicts` (after each 5-min verdict) | `{"date":"YYYY-MM-DD","verdicts":{index:[{t:"HH:MM",behavior,emoji,tone,action,ce5,pe5,strad}]}}` — reloaded at startup only if same date |
...
- The control bar is split into **two per-bot blocks** (`.tb-grp`) so it is unambiguous which controls drive which bot: `.tb-grp.p10` (cyan left rail + tint, header `🚀 PROD10 — MANUAL BOT`) holds the PROD10 SETUP row, and `.tb-grp.auto` (purple left rail + tint, header `⚡ MOMENTUM AUTO BOT`, sub-line "SETUP, RISK and FILTERS below apply to this bot only — not to PROD10") holds all three Auto rows plus the VIX Auto Config panel. Row tags inside a block render as `.tb-rowtag.sub` with a `└` prefix, so they read as children of the block header rather than as peers of it.
- `⚡ AUTO` sub-panel (Momentum bot launcher) — **three wrapping rows** (`.tb-cbar-row.wrap`, `row-gap:9px`, no horizontal scroll) so every control is visible without scrolling:
  1. `⚡ AUTO` — `mb-index, mb-expiry, mb-lots, mb-exit-mode, mb-target-pts (TARGET PTS / TRAIL @ PTS — label follows exit mode), TRADE MODE (PAPER|MOCK|LIVE), mb-validate-btn, mb-prem-min/max, mb-strikes, mb-scan-sec, mb-poll-sec` + `🚀 Auto Bot`
  2. `🛡 RISK` — `mb-place-tgt-btn (PLACE TGT), mb-cooldown-sec (COOLDOWN S), mb-nosig-sec (NO-SIG S), mb-hard-sl-btn + mb-hard-sl-pts (HARD SL), mb-atr-sl-btn (ATR SL), mb-atr-src-btn (ATR SRC), mb-sl-mult (SL MULT), mb-sl-floor (SL FLOOR)`
  3. `🎚 FILTERS` — `CHOP, CONS SL, MIN SCORE, VEL FILTER, VEL %, CONS %, VIX AUTO` + capital calculator

  **Stop mode is exclusive** (`_mbPaintSLMode`): HARD SL and ATR SL can never both be on. Turning HARD SL on forces `_mbAtrSL=false`, enables the inline `mb-hard-sl-pts` input (points typed in the UI — no browser prompt) and dims/disables ATR SRC, SL MULT and SL FLOOR; clicking ATR SL while HARD SL is on is refused with an alert. Both the launch config and `_mbPushConfig` send `HARD_SL_ATR_BASED: _mbHardSL ? false : _mbAtrSL`, and hydration prefers the fixed stop when the override file somehow holds both — the same precedence the bot applies. `🚀 Auto Bot` → `POST /api/bot/start {id:"momentum", config:{trade_mode,index,expiry,lots,exit_mode,min_premium,max_premium,atm_range,validate_orders,scan_seconds,poll_seconds,TRAIL_START_PROFIT,cooldown_sec,no_signal_wait_sec,HARD_SL_ATR_MULTIPLIER,HARD_SL_POINTS,place_target_order,HARD_SL_FIXED,HARD_SL_FIXED_POINTS,choppiness_enabled,consec_sl_brake,HARD_SL_ATR_BASED,atr_source,min_score_filter,velocity_filter}}`; LIVE mode requires a JS `confirm()`. The three timing inputs also push live on `change` via `mbPushTiming()` → `POST /api/momentum/config` (input flashes green/red), and `_mbPushConfig()` (toggle handler) merges them in too. Capital calculator uses `/api/groww_capital` + `/api/lot_size`.
...
Header (always visible): `#htitle`, `#mkt-ticker` (NIFTY/BANKNIFTY/SENSEX cards, click-to-pin primary, order persisted in `localStorage['idx_order']`, polled `/api/indices` every **1 s**), `#htime` live clock (1 s), `#mtc-badge` market countdown to 15:30 / "Opens Xh Ym" (weekday + 9:15–15:30 aware), `Refresh #countdown` (`const R=15`, `load()` hits `/api/data` when it reaches 0), theme colour picker (`#picker-btn`, BUY CE/PE colour + glow persisted in localStorage), notification bell `#notif-badge`/`#notif-panel`/`#notif-mute-btn` (polls `/api/alerts` every **5 s**, WebAudio beeps per alert type, ≤60 items, unread counter), `#bbar` bot status strip, and `#hdr-ver` — the current app version chip (`v<current_version>` from `/api/release_notes`), which expands and scrolls to the release-notes card on click (`rnJumpToNotes()`).

### AUTO-FIT signal engine (`_autofit_loop`, 180 s)

One sample every 3 minutes, market hours only (09:15–15:30 weekdays; outside that a `CLOSED`
sample is set as `current` and **not** appended to history). Inputs, two API calls per sample:

| Component | `kind` | Input | Amber / Red |
|---|---|---|---|
| Volatility expansion | tape | median range of the last 5 × 1-min NIFTY bars ÷ median of the prior 55 (`_fetch_index_candles`, phantom 09:00 bar filtered) | 1.35 / 1.80 |
| Shock bar | tape | biggest of the last 3 bars ÷ that same baseline | 2.00 / 3.00 |
| Premium swing | tape | largest \|ce5\|/\|pe5\| in the newest `.premium_pulse_verdicts.json` entry | 4 / 8 pts |
| ATR ÷ target | **setup** | 14-period EMA ATR of the ATM CE from 5-min candles ÷ `TRAIL_START_PROFIT` | 3 / 5 — **`caps_at:1`** |
| Hard SLs (30 min) | damage | today's `trade_history` Auto rows with `HARD SL` in `exit_reason` | 1 / 2 |
| Peak giveback | damage | ₹ handed back from the day's peak cumulative Auto P&L, as % of peak | 15% / 25% |

State = worst component level after `min(level, caps_at)` — so the setup component can only
raise the light to AMBER; a permanently-wrong target/ATR geometry must not pin it RED all day
and destroy the timing information. A RED latches for `AUTOFIT_RED_COOLOFF = 5` samples (15 min);
a latched sample carries `cooling: true`.

Validated on 2026-09-11's real tape: the light sat AMBER, went **RED 13:42 → 14:21** — twelve
minutes before the −₹67k loss cluster at 13:54/13:55/14:02 — and produced only 5 state changes
across the session. Advisory only: no bot reads it.

Tabs (`switchTab(id, btn)`):
1. **📡 Live Dashboard** — **release-notes card first** (`.rn-card`): clickable header with `📋 RELEASE NOTES`, the current-version pill `#rn-current-ver`, a summary line (release + custom-note counts) and a collapse chevron whose open/closed state persists in `localStorage['rnOpen']`. Body lists every release newest-first — version, date, title, a `current` pill on the newest, `sections[]` rendered as headed bullet lists, then a **My notes** block of user notes (timestamp, text, ✕ delete). Footer row adds a note (version dropdown + text input, Enter or `＋ Add note`) or a new version (`🏷 New version` → prompts). All of it is rendered by `rnRender()` from `/api/release_notes` and every value is escaped through `rnEsc()`. Then: consensus box (`#csig/#csmry/#csrc/#cbull/#cbear`), Scalp Plan card with ON/OFF toggle (`scalp`), Key Levels table (`#lvlbody`, `#swing-danger`), Master Signal card, Pivot card (`/api/pivots` every **60 s**, source badge), Fibonacci card, Option Signal card, Premium tracker card, Trade Bot card, Signal Monitor card, AI Summary card with toggle (`ai`) + source badge.
...
4. **🎯 Auto Signal** (`initAutoFitTab`) — dedicated page for the Momentum Auto Bot's AUTO-FIT light. Hero card (state word, headline, recommended action, trigger list, NIFTY spot, ATR÷target, sample time, live countdown to the next 3-minute sample, manual `↻ Refresh view`), a standing advisory note, one card per component (value, amber/red thresholds, `kind` tag, `caps at AMBER` pill where applicable), the day's timeline as a coloured cell strip (one cell per sample, hover = time/state/triggers, last cell outlined) with green/amber/red minute totals, a **state changes** table (time, from → to, held-for, triggered-by) and a collapsible **all samples** table (newest first). View polls `/api/autofit` every 20 s; the engine itself samples every 180 s.
5. **💹 PnL Status** — today's P&L card (big number, count/wins/losses/open, unrealised, total incl. open), daily-target progress bar with editable target (`localStorage['nifty_pnl_target']`) and alarm toggle, TRADING PERMISSION card (verdict, permission score bar, market score bar, recent win rate, direction bias, position size, breakdown), LIVE MARKET card (NIFTY/VIX/Gap/PCR/market open, market-condition score + breakdown), 3-year stats tiles, trade-history table with from/to date filters (`/api/trade_history`), Personal AI runner (`/api/personal_ai`, `/api/personal_ai/run`, polled while running).
6. **📈 Performance** — two tables from `/api/performance` (30 s): S/R level events (Time, Level, Label, Type, Near at, Result) and option-signal events (Time, Spot, Reason, Max Move, Result).
7. **🤖 Bot Control** — cards generated from `/api/bot/registry`; per-card status badge, Start/Stop (`/api/bot/start|stop`), inline log pane for non-terminal bots (`/api/bot/logs?id&n=30`), momentum radio group `momentum-mode` → `config.trade_mode`; Start All / Stop All; status refresh every **4 s**.
8. **🔭 Scanner** (Trendline) — status badge, start/stop `trendline_scanner`, config form (expiry from `/api/trendline_expiries`, premium min/max, lots, toggles: Ascending/Descending/Horizontal TL, Spot Confirm, Volume Surge + multiplier, % Confirm + value) saved via `POST /api/trendline_config`, "Apply Ideal" preset, today's stats (trades/wins/losses/PnL), active trade panel, signals list (`/api/trendline_signals`), log viewer (`/api/bot/logs?id=trendline_scanner&n=80`), live status panel (spot, bars, TL count, in-range, watching), NIFTY + option chart from `/api/trendline_chart`, demo-guide modal, backtest section (from/to/mode → `/api/trendline_history`; fresh run → `/api/run_trendline_backtest`) with summary + trades table. Polls every **8 s**.
9. **🧠 AI Brain** — Quick Summary block (`#qs-text`, toggle `qs_ai`, manual `POST /api/qs_ai/refresh`, poll `/api/mb_ai` every **2.5 s** while running, auto every 5 min) and the dual AI Brain report (`intraday`, `longterm`, `key_levels`, `risks`, `bottom_line`, ts, `context_lines`), toggle `mb_ai`, manual `POST /api/mb_ai/refresh`, auto refresh timer 300000 ms; OFF state instructs `claude login`.
10. **🌡 VIX** — Market Regime cards: NIFTY DAY RANGE (source priority: `snap.clean_ohlc` — today's phantom-filtered H/L from `read_clean_day_ohlc()` (regime-history 15-min candle row for today, widened by `_ltp_day_extremes` — NIFTY session extremes tracked from the 3s LTP stream between 09:15–15:30; non-blocking, refreshes via `_build_regime_history_async()`, 5-min failure backoff `_regime_hist_fail_ts`) → fibo day_high/low → `mkt_idx._ohlc.nifty`; suffix ` (filtered)`/` (live)` shows the source; labels WIDE ≥1.5 %, MODERATE ≥0.75 %, else NARROW — fibo & Quote-API H/L carry the phantom 09:00 wick, which used to false-trigger `bothSideWhipsaw` so the badge said CHOP while the score said SIDEWAYS), ATM STRADDLE (`oi_snapshot.atm_momentum.ce_ltp+pe_ltp`; HIGH IV >200, NORMAL >120, else LOW), PREMIUM ACTIVITY (`ce_ltp_chg`/`pe_ltp_chg`; HIGH ≥8, MODERATE ≥3, else STAGNANT; bias inferred from signs), REGIME SCORE 0–6 = `rangeScore(0-2) + premScore(0-2) + vixScore(0-2)` where `vixScore = 2 if vix≥15 or sessΔ≥2% ; 1 if vix≥12 or sessΔ≥0.8%`, plus `bothSideWhipsaw` (range ≥0.8 % with premium move <3 and a falling side), `premCrush` (both deltas <0), IV-squeeze proxy (`avgATMIV < VIX/√252 × 0.8`). Verdict labels: `⚡ THETA DECAY`, `↔ BOTH-SIDE CHOP`, `— SIDEWAYS` (≤1), `〜 MIXED` (≤3), `↗ TRENDING` (≤5), `🔥 STRONG TREND`. VIX panel: current + regime (`ULTRA CALM <12, CALM <15, MODERATE <18, ELEVATED <22, DANGER ≥22`), session Δ%, 10-min Δ% (index `len-6`), session high/low/open/range, 30-min velocity, tick count, timestamp, hover sparkline over `vix_history`; empty-state messages differ for closed market vs NSE rate-limiting. **15-DAY REGIME HISTORY** table (`#rh-table`/`#rh-summary`, fetch `/api/regime_history` on load + every 30 min): server rebuilds daily NIFTY OHLC from **15-min candles** (`_daily_ohlc_from_15m`: drops bars before 09:15 — Groww's phantom 09:00 wick — then bars with range > 8× the day's median), VIX close from `NSE-INDIAVIX` 1-day candles; per day computes range pts/%, move (close−open), efficiency = |close−open|/(high−low), gap %, VIX day-over-day Δ%; `_classify_regime_day`: range ≥1.0 % & eff ≥0.6 → `🔥 STRONG TREND UP/DOWN` · range ≥0.55 % & eff ≥0.5 → `↗/↘ TRENDING` · range ≥0.75 % & eff <0.3 → `↔ BOTH-SIDE CHOP` · range <0.45 % → `— SIDEWAYS` · else `〜 MIXED` (thresholds calibrated to phantom-filtered ranges, NOT the intraday score's 0.75/1.5). Today's row is flagged `partial` ("live"), close refreshed from `_idx_state.nifty.last`; H/L stay candle-derived because the Quote-API OHLC carries the phantom wick. **ATM PREMIUM Δ** per row: strike = day open rounded to 50; weekly expiry = next Tuesday (±1-day candidates for holiday shifts); `_atm_premium_day()` fetches that day's CE+PE 15-min candles via `_fetch_day_option_candles()` — works for EXPIRED contracts (`NSE-NIFTY-{DDMonYY}-{strike}-{CE|PE}`), sequential with 0.25s pacing + one 429 retry (parallel bursts get rate-limited, never use a ThreadPool here; full build ≈ 20s). Fields: atm_strike, expiry, ce/pe_chg, ce/pe_chg_pct (open→close), straddle_open/close, prem_label/prem_key — CRUSH (both down) · EXPANSION (both up) · CE PAID/PE PAID (one side up ≥5%) · DECAY (winning side <5%) — plus activity fields: `prem_dir_pct` = (CEΔ−PEΔ)/straddle_open×100 (net directional flow), `prem_activity` = HIGH ≥35% / MODERATE ≥15% / STAGNANT (daily analog of the live PREMIUM ACTIVITY card; theta days bleed both sides equally → dir≈0), `prem_bias` = BULLISH/BEARISH/NEUTRAL at ±15%. Display-only; regime classification stays OHLC+VIX. Colors: strong `#ef4444`, trend `#4ade80`, chop `#f59e0b`, side `#64748b`, mixed `#a78bfa`.
11. **💓 Premium Pulse** — `initPulseTab()` fetches `/api/premium_pulse?index=<_ppIdx>` on open + every **20 s**, `/api/premium_pulse_decision` every **3 s** (one fetch renders both the Final Call and the momentum cards), and `/api/premium_pulse_chart` every **3 s** (`_ppTimer`/`_ppFastTimer`/`_ppChartTimer`, all gate on `.active`); backend LTP sampling runs at 2 s so cards are ≤~3 s behind the exchange. Topmost block = **CE/PE momentum cards** (`#pp-momo-ce/pe`: LTP + 5m/15m %, `⚡ MOMENTUM`/`QUIET` tag; card blinks via `.pp-momo-live-ce/pe` keyframe glow when `momo.*_on`). Chart: 12 minor horizontal gridlines (major+label every 4th), vertical lines at each x-tick + midpoints; **point inspector** — two readouts in `#pp-chart-info`: **⚡ AUTO** (always on): `_ppAutoSel` pins A = latest chart point and B = first point ≥3 min earlier (gap-safe), yellow dashed markers, `Δ 3 min` CE/PE change in % and ₹, live with every 3 s redraw; **🖱 MANUAL** (unchanged): `_ppChartClick` maps offsetX→index (`_PP_CHART_PAD`), pins marker A (dashed vline + CE/PE dots), second click pins B and shows A→B Δ% and Δ₹ per side, third click resets to a fresh A, `✕ clear` (`_ppClearSel`) returns to auto-only; manual markers replace auto markers on the canvas while set, both readout lines stay visible; selections stored in `_ppSel` by `t` (HH:MM) so they survive the 3 s redraws (`renderPulseSel` drops stale times). Next block = **🎯 FINAL CALL**: live 5s decision (`#pp-dec-label` colored by tone + `dec:<KEY>` tooltip, why line, "this call since HH:MM:SS · held Xm Ys") beside the LAST 1 HOUR call timeline (`#pp-dec-history`, span rows: start · duration · call, each with its `dec:` tooltip); `renderPulseDecision()` also first-paints from the main payload's `decision`/`decision_history`; closed-market and warming-up empty states. NIFTY/SENSEX picker (`setPulseIdx`), contract line (ATM strike · expiry · spot). Verdict card: behavior label colored by `tone` (`bull/bear/warn/info/dim`), confidence chip, action paragraph, reason bullets, ATM straddle + CE/PE LTPs. Windows grid (SINCE OPEN / LAST 1 HOUR / LAST 15 MIN / LAST 5 MIN): per window CE/PE/STRAD Δ% + Δ₹ (green/red by sign), cls badge (FLAT dim · SLOW info · STEADY warn · FAST bear), spot Δ% and path tag (`one-way ≥0.6 · mixed ≥0.35 · whipsaw` from min CE/PE efficiency). Canvas chart `#pp-chart` (dpr-scaled, CE `--bull` / PE `--bear`, 4 gridlines, ~6 x-labels). Trap-spikes card, decay-check card, and the 5-min verdict log table (time, behavior, CE 5m %, PE 5m %, straddle, action) — how the day evolved. **Tooltips**: every element carries `data-pptip="<key>"` → global fixed `#pp-tooltip` (mr-tooltip styling, `cursor:help` via scoped `<style>`); **delegated** `document`-level mouseover/mousemove (cards re-render every 20 s so per-element binding like `data-mrtip` won't survive). `_ppTipHtml(key)`: `beh:<NAME>` keys (label parenthetical stripped via `split(' (')[0]`) → `_ppBehTips` — 7 detailed meaning+WHAT-TO-DO entries (TRAP / WHIPSAW, PREMIUM DECAY, FAST MOMENTUM, FAST BUT CHOPPY, STEADY TREND, SLIGHT MOVEMENT, FLAT); all other keys → `_ppTips` (idx, contract, verdict, conf, straddle, ltps, action, reasons, refresh, chart, traps, decay, history, hist-*, win-day/60m/15m/5m with per-window threshold tables, row-ce/pe/strad, path, cls:FLAT/SLOW/STEADY/FAST). Helpers `_ppTipHead/_ppTipDo/_ppTipGrid` build the HTML.
12. **⚡ Decision Engine** — header badges (mode, running, profile, events file); LAUNCH CONTROL form (`dec-mode` shadow/live, `dec-profile`, `dec-index`, `dec-expiry` from `/api/engine/expiries`, `dec-lots`, `dec-pmin`, `dec-pmax`, `dec-validate`, `dec-live-confirm` "Type YES to arm LIVE" shown only for live) → `/api/engine/start`; Stop button with a `confirm()` warning → `/api/engine/stop`; console pane polling `/api/engine/console?n=40` every **3 s**; cards for latest decision, rejection gates (count + %), per-engine pass %/avg score in fixed order `trend, market_structure, support_resistance, premium_momentum, breakout, market_strength, option_selection, volatility, trading_rules, risk, signal_stability`, aggregate stats, last 10 trades, strategy config; offline message pointing at `trading_decision_engine/logs/events_*.jsonl`.
13. **🛡 Control** — iframe of the port-8790 panel (§11) with fallback hint.
14. **🗺️ Guide** — static documentation of the bot ecosystem, data sources (`ayush_previous_data/*.xlsx`, `Lakshmi.xlsx`, `instrument.csv`), and workflows.
