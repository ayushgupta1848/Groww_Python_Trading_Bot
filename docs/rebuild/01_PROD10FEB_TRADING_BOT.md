...
### 4.0.4 `get_order_executed_price(order_id, access_token, segment="FNO", expected_qty=None)` → `(avg_price, total_qty)`
- PAPER: return `(_paper_orders[id]["price"], ["qty"])`, print `📋 [PAPER] Executed: ₹{p:.2f} × {q} (order {id})`.
- **`expected_qty`** = the authoritative fill quantity. Every call site passes the qty it ordered (an EXECUTED order is fully filled). If `None`, learn it first from `GET /v1/order/status/{id}` → `int(payload.filled_quantity)` (best-effort, `timeout=5`; stays `None` on error).
- `GET https://api.groww.in/v1/order/trades/{order_id}?segment={segment}&page=0&page_size=50` (bare `requests.get`, `timeout=5`).
- **Retry schedule `_backoff = (0, 0.25, 0.5, 1.0, 1.25, 1.5, 2.0)`** — 7 attempts; `time.sleep(delay)` *before* attempts 2-7 (first fires immediately). Retry on exception, on `data["status"] != "SUCCESS"`, on empty `payload.trade_list`, **or on an incomplete trade list** (`expected_qty` truthy and `Σ t["quantity"] < expected_qty` — large orders fill as multiple exchange trades and the lagging endpoint can expose only the first fills; remember the partial `(avg_price, qty_seen)`, print `⚠️ Trade list incomplete: {seen}/{expected} qty visible (attempt i/7) — retrying...`).
- Success (`total_qty` ≥ `expected_qty`, or no `expected_qty` known): `total_qty = Σ t["quantity"]`; `avg_price = round(Σ(t["price"]*t["quantity"]) / total_qty, 2)`; prints `✅ {transaction_type} {trading_symbol} | Total Qty={q} | Avg Price=₹{p}`.
- **Partial fallback** after all 7 attempts, if an incomplete list was seen: return `(partial_avg_price, expected_qty)` — the position SIZE is authoritative, the partial avg is the best price estimate. Print `⚠️ Trades endpoint showed only {seen}/{expected} qty after 7 attempts — using avg ₹{p} from visible fills for FULL qty {expected}.`
- **Status fallback** otherwise: `GET /v1/order/status/{id}?segment={segment}` → `payload.average_price or payload.avg_price`, `payload.filled_quantity or payload.quantity`; if both truthy, return `(round(float(p),2), int(q))` and print `✅ Fallback via order-status: …`. Else print `⚠️ Order-status fallback had no average_price: {payload}` and **return `(None, None)`**.
- *(Incident 2026-08-11 13:09: a 1430-qty market BUY filled as ~11 trades; the first trades fetch saw one 130-qty fill, the bot set `quantity=130` and left 1300 qty unmanaged. Never trust a non-empty trade_list without checking it against filled_quantity.)*
...
| 8 | GET | `/v1/order/trades/{order_id}?segment=FNO&page=0&page_size=50` | fills (timeout 5 s, 7 attempts; retried until Σqty ≥ expected fill qty) | `payload.trade_list[].{price, quantity, trading_symbol, transaction_type}` |
| 9 | POST | `/v1/order/cancel` body `{"segment":"FNO","groww_order_id":"<id>"}` | cancel resting limit sell (timeout 8 s) | `success`, `status=="SUCCESS"`, or `payload.order_status` in `CANCELLED`/`CANCELLATION_REQUESTED` |
