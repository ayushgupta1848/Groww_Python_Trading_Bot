...
CONFIG = {
    "index": "NIFTY",
    "expiry": "2025-12-23",  # Updated to DD/MM/YYYY to match instruments JSON
    "min_premium": 80,
    "max_premium": 130,
    "lots": 14,
    "book_profit": 1050,
    ...
    "TRAIL_START_PROFIT": 1.5,  # Start trailing after this profit per unit (in points)
...
def place_cp_order(command, is_auto=False):
    ...
    if is_auto:
        ...
        hard_sl = avg_price - CONFIG.get("HARD_SL_POINTS")
        ...
        while True:
            ...
            if time.time() - start_time >= max_time:
                ...
                ltp_now = get_ltp_for_instrument(instrument, access_token, verbose=False) or avg_price
                profit = (ltp_now - avg_price) * qty
                log_trade_to_excel(
                    instrument.get("internal_trading_symbol"),
                    avg_price, ltp_now, qty, profit
                )
                break
        ...
        while True:
            ...
            if time.time() - start_time >= max_time:
                ...
                ltp_now = get_ltp_for_instrument(instrument, access_token, verbose=False) or avg_price
                profit = (ltp_now - avg_price) * quantity
                log_trade_to_excel(
                    instrument.get("internal_trading_symbol"),
                    avg_price, ltp_now, quantity, profit
                )
                break
