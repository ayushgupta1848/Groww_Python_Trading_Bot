# ENHANCED_ManualBOT_groww_option_trading_final_bot.py
# Enhanced Engine — adds VWAP/RSI/ADX entry filters, OI/PCR check,
# dynamic ATR-based trailing step, partial booking (50% at 1.5×ATR),
# and VWAP-cross exit to improve win rate over the original PROD10FEB bot.
...
CONFIG.update({
    # --- Entry Filters ---
    "ENABLE_ENTRY_FILTERS": True,   # Master switch; set False to disable all entry checks

    "ENTRY_RSI_CE_MIN": 40,         # CE entry blocked if RSI below this (not enough bullish momentum)
    "ENTRY_RSI_CE_MAX": 70,         # CE entry blocked if RSI above this (overbought)
    "ENTRY_RSI_PE_MIN": 30,         # PE entry blocked if RSI below this (oversold)
    "ENTRY_RSI_PE_MAX": 60,         # PE entry blocked if RSI above this (not enough bearish momentum)

    "ENTRY_ADX_MIN": 20,            # Minimum ADX to confirm a trend is in play (< 20 = choppy)

    "ENTRY_VWAP_CHECK": True,       # Option LTP must be above its own VWAP at entry (momentum check)

    # --- OI / PCR Filter ---
    "ENABLE_OI_CHECK": True,        # Check PCR at ATM strike before entry
    "OI_PCR_CE_MAX": 1.4,           # Reject CE trade if PCR > 1.4 (too many puts = strong bearish positioning)
    "OI_PCR_PE_MIN": 0.7,           # Reject PE trade if PCR < 0.7 (too many calls = strong bullish positioning)

    # --- Dynamic Trailing ---
    "DYNAMIC_TRAIL_STEP": True,     # Trail step adapts to ATR instead of fixed value
    "DYNAMIC_TRAIL_ATR_MULT": 0.5,  # Trail step = 0.5 × ATR
    "TRAIL_STEP_MIN": 0.50,         # Minimum trail step (prevent over-tight trailing on calm days)
    "TRAIL_STEP_MAX": 2.00,         # Maximum trail step (prevent too-wide trailing on volatile days)

    # --- Partial Booking ---
    "ENABLE_PARTIAL_BOOKING": True, # Sell 50% of position when profit reaches threshold
    "PARTIAL_BOOKING_ATR_MULT": 1.5,# Book 50% when profit >= 1.5 × ATR per unit

    # --- VWAP Exit ---
    "ENABLE_VWAP_EXIT": True,       # Exit remaining position when LTP crosses back below entry-time VWAP (while in profit)

    # --- Entry Filter Mode ---
    "ENTRY_FILTER_STRICT": False,   # True = auto-block trade on filter fail | False = warn and ask user
})
...
# ==================== ENHANCED ENTRY FILTER FUNCTIONS ====================

def check_entry_filters(instrument, option_type, entry_ltp):
    """
    Runs VWAP, RSI, ADX checks on the option's own technicals.
    Returns: (passed: bool, reasons: list[str], techs: dict|None)
    """
    if not CONFIG.get("ENABLE_ENTRY_FILTERS", True):
        return True, [], None

    print(f"🔬 Running entry filters for {option_type}...")
    techs = get_technicals(instrument["groww_symbol"], groww, segment="FNO", instrument=instrument)

    if not techs:
        print("⚠️ Technicals unavailable — entry filters skipped")
        return True, ["technicals_unavailable"], None

    rsi  = techs.get("rsi", 50)
    adx  = techs.get("adx", 25)
    vwap = techs.get("vwap")
    atr  = techs.get("atr", 0)
    ...
    # RSI check
    if option_type == "CE":
        rmin, rmax = CONFIG["ENTRY_RSI_CE_MIN"], CONFIG["ENTRY_RSI_CE_MAX"]
    else:
        rmin, rmax = CONFIG["ENTRY_RSI_PE_MIN"], CONFIG["ENTRY_RSI_PE_MAX"]

    if not (rmin <= rsi <= rmax):
        reasons.append(f"RSI {rsi:.1f} not in [{rmin}–{rmax}] for {option_type}")

    # ADX check
    adx_min = CONFIG["ENTRY_ADX_MIN"]
    if adx < adx_min:
        reasons.append(f"ADX {adx:.1f} < {adx_min} (market too choppy)")

    # VWAP check — option LTP must be above its own VWAP (trending up)
    if CONFIG.get("ENTRY_VWAP_CHECK", True) and vwap and vwap > 0:
        if entry_ltp < vwap:
            reasons.append(f"LTP ₹{entry_ltp:.2f} < VWAP ₹{vwap:.2f} (option below average price)")

    passed = len(reasons) == 0

    status = "✅ PASSED" if passed else f"❌ FAILED ({len(reasons)} condition{'s' if len(reasons) > 1 else ''})"
    print(f"🔬 Entry Filter: {status}")
    print(f"   RSI={rsi:.1f}  ADX={adx:.1f}  VWAP={vwap:.2f if vwap else 'N/A'}  ATR={atr:.2f}  LTP={entry_ltp:.2f}")
    for r in reasons:
        print(f"   ⚠️  {r}")

    return passed, reasons, techs


def check_oi_pcr(underlying, expiry, option_type, access_token):
    """
    Checks PCR (Put/Call Ratio) at ATM to filter directionally wrong trades.
    Returns: (passed: bool, pcr: float|None)
    """
    if not CONFIG.get("ENABLE_OI_CHECK", True):
        return True, None

    try:
        payload = _get_full_option_chain_cached(underlying, expiry, access_token)
        strikes = payload.get("strikes", {})

        # Find ATM strike
        spot = CONFIG.get("spot", 0)
        step = 100 if "SENSEX" in underlying.upper() else 50
        atm = round(spot / step) * step

        atm_data = strikes.get(str(atm)) or strikes.get(str(int(atm)))
        if not atm_data:
            print(f"⚠️ OI check: ATM strike {atm} not found in chain — skipping")
            return True, None

        ce_oi = atm_data.get("CE", {}).get("open_interest", 0) or 0
        pe_oi = atm_data.get("PE", {}).get("open_interest", 0) or 0

        if ce_oi == 0:
            return True, None  # No CE OI data, skip check

        pcr = pe_oi / ce_oi

        if option_type == "CE":
            limit = CONFIG["OI_PCR_CE_MAX"]
            passed = pcr <= limit
            verdict = "✅" if passed else "❌"
            print(f"📊 OI/PCR Check {verdict}: PCR={pcr:.2f} (CE limit ≤{limit}) | CE_OI={ce_oi:,} | PE_OI={pe_oi:,}")
        else:
            limit = CONFIG["OI_PCR_PE_MIN"]
            passed = pcr >= limit
            verdict = "✅" if passed else "❌"
            print(f"📊 OI/PCR Check {verdict}: PCR={pcr:.2f} (PE limit ≥{limit}) | CE_OI={ce_oi:,} | PE_OI={pe_oi:,}")

        return passed, pcr

    except Exception as e:
        print(f"⚠️ OI check error: {e} — skipping")
        return True, None


def compute_dynamic_trail_step(atr):
    """Returns ATR-scaled trailing step, clamped to configured min/max."""
    if CONFIG.get("DYNAMIC_TRAIL_STEP", True) and atr and atr > 0:
        step = atr * CONFIG.get("DYNAMIC_TRAIL_ATR_MULT", 0.5)
        return max(CONFIG.get("TRAIL_STEP_MIN", 0.50), min(CONFIG.get("TRAIL_STEP_MAX", 2.00), step))
    return CONFIG["TRAIL_STEP"]
...
def place_cp_order(command, is_auto=False):
    ...
    if is_auto:
        ...
        opt_type_for_filter = parsed_symbol_details.get("opt_type", "CE").upper()
        filter_passed, filter_reasons, entry_techs = check_entry_filters(instrument, opt_type_for_filter, entry_price)

        # OI/PCR check
        oi_passed, pcr_value = check_oi_pcr(
            parsed_symbol_details["underlying"], parsed_symbol_details["expiry_date"],
            opt_type_for_filter, access_token
        )
        if not oi_passed:
            filter_passed = False
            filter_reasons.append(f"OI/PCR unfavourable for {opt_type_for_filter} (PCR={pcr_value:.2f})")

        if not filter_passed:
            if CONFIG.get("ENTRY_FILTER_STRICT", False):
                print(f"🚫 Trade BLOCKED by entry filters.")
                return
            else:
                confirm = input(f"⚠️  Entry filters FAILED. Proceed anyway? (y/n): ").strip().lower()
                if confirm != "y":
        ...
        # ── ENHANCED: Use entry_techs (from filter check) for ATR/VWAP ──
        # Fall back to background-fetched ATR if entry filters were skipped
        atr = CONFIG["HARD_SL_POINTS"]
        entry_vwap = None
        if entry_techs:
            if entry_techs.get("atr"):
                atr = entry_techs["atr"]
                print(f"✅ ATR (from entry filter fetch): {atr:.2f}")
            entry_vwap = entry_techs.get("vwap")
        else:
            # Fallback: wait briefly for background thread
            try:
                ...
                if not result_queue.empty():
                    ...
                    if techs and techs.get("atr"):
                        atr = techs["atr"]
                        print(f"✅ ATR fetched (background): {atr:.2f}")
                    if techs and techs.get("vwap"):
                        entry_vwap = techs["vwap"]
            except Exception:
                pass
        ...
        # ── ATR-based SL (fix: guard against zero ATR) ──
        if atr and atr > 0:
            hard_sl = round(avg_price - (1.5 * atr), 2)
        else:
            hard_sl = round(avg_price - CONFIG["HARD_SL_POINTS"], 2)

        # ── Dynamic trail step ──
        trail_step = compute_dynamic_trail_step(atr)

        # ── Partial booking state ──
        partial_booked = False
        remaining_qty = quantity
        partial_qty = quantity // 2
        partial_book_threshold = avg_price + CONFIG.get("PARTIAL_BOOKING_ATR_MULT", 1.5) * (atr if atr > 0 else CONFIG["HARD_SL_POINTS"])

        print(f"📈 Trailing started... SL: ₹{hard_sl:.2f} | Trail step: {trail_step:.2f} | ATR: {atr:.2f}")
        print(f"📊 Partial booking at ₹{partial_book_threshold:.2f} (+{partial_book_threshold - avg_price:.2f} pts) for {partial_qty} units")
        if entry_vwap:
            print(f"📊 VWAP exit armed at ₹{entry_vwap:.2f} (triggers only while in profit)")
        try:
            send_telegram(f"📈 Trailing started... Dynamic SL: {hard_sl:.2f}")
        except:
            pass

        # ⚡ ENHANCED monitoring loop — partial booking + VWAP exit + LTP fail-safe
        current_ltp = avg_price
        while True:
            if time.time() - last_heartbeat >= 30:
                print(f"💓 Monitoring... LTP: ₹{current_ltp:.2f} | High: ₹{highest_price:.2f} | Remaining: {remaining_qty}")
                last_heartbeat = time.time()
            ...
            if ltp is None:
                ltp_fail_count += 1
                if ltp_fail_count >= 20:
                    sell_reason = "❌ LTP feed lost — emergency exit"
                else:
                    time.sleep(poll)
                    continue
            else:
            ...
            # ── ENHANCED EXIT 1: Partial booking (sell 50% at 1.5×ATR profit) ──
            if not partial_booked and sell_reason is None and CONFIG.get("ENABLE_PARTIAL_BOOKING", True):
                if ltp >= partial_book_threshold and partial_qty > 0:
                    try:
                        print(f"📊 PARTIAL BOOK: Selling {partial_qty} units @ ₹{ltp:.2f}")
                        place_market_order_groww(instrument, partial_qty, "SELL", "MIS")
                        partial_booked = True
                        remaining_qty = quantity - partial_qty
                        partial_profit = (ltp - avg_price) * partial_qty
                        print(f"✅ Partial sell done | Locked ₹{partial_profit:.2f} | Remaining: {remaining_qty} units")
                        send_telegram(f"📊 PARTIAL BOOK {partial_qty}u @ ₹{ltp:.2f} | Locked ₹{partial_profit:.2f} | {remaining_qty} running")
                        # Tighten trail step after booking partial profit
                        trail_step = max(CONFIG.get("TRAIL_STEP_MIN", 0.50), trail_step * 0.7)
                        print(f"📉 Trail step tightened to {trail_step:.2f} after partial booking")
                    except Exception as pb_err:
                        print(f"⚠️ Partial booking failed: {pb_err}")

            if sell_reason is None:
                # ── ENHANCED EXIT 2: VWAP cross-below exit (only while in profit) ──
                if CONFIG.get("ENABLE_VWAP_EXIT", True) and entry_vwap and entry_vwap > 0:
                    if ltp < entry_vwap and ltp > avg_price:
                        sell_reason = f"📉 VWAP EXIT: LTP ₹{ltp:.2f} crossed below VWAP ₹{entry_vwap:.2f}"

            if sell_reason is None:
                # ── Hard SL ──
                if ltp <= hard_sl:
                    sell_reason = f"🛑 DYNAMIC SL HIT @ ₹{ltp:.2f}"
                # ── Time exit ──
                    ...
                    if ltp > highest_price:
                        ...
                        try:
                            send_telegram(f"🔼 New High: ₹{highest_price:.2f}")
                        except Exception:
                            pass
                    ...
                    if highest_price >= avg_price + trail_start:
                        ...
                        if trail_exit != last_trail_exit:
                            print(f"📉 Trail Active | LTP=₹{ltp:.2f} | High=₹{highest_price:.2f} | Exit=₹{trail_exit:.2f} | Step={trail_step:.2f}")
                            try:
                                send_telegram(f"📉 Trail | LTP=₹{ltp:.2f} | Exit=₹{trail_exit:.2f}")
                            except Exception:
                                pass
                            last_trail_exit = trail_exit

                        if ltp <= trail_exit:
                            sell_reason = f"🔻 Trailing HIT @ ₹{ltp:.2f}"

            # ── Place exit SELL for remaining quantity ──
            if sell_reason:
                print(sell_reason)
                print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}]")
                sell_qty = remaining_qty  # Only sell what's left after any partial booking

                if "SL HIT" in sell_reason or "DYNAMIC SL" in sell_reason or "feed lost" in sell_reason:
                    play_sound_async(SOUND_SL)
                else:
                    play_sound_async(SOUND_PROFIT)

                try:
                    print(f"🔄 Placing SELL order for {sell_qty} units @ market price...")
                    order_resp = place_market_order_groww(instrument, sell_qty, "SELL", "MIS")
                    ...
                    if CONFIG.get("VALIDATE_ORDERS", True) and sell_order_id:
                        ...
                        if sell_status in ["EXECUTED", "COMPLETED", "DELIVERY_AWAITED"]:
                            sell_price, sold_qty = get_order_executed_price(sell_order_id, access_token, expected_qty=sell_qty)
                            if sell_price and sold_qty:
                                profit = (sell_price - avg_price) * sold_qty
                                print(f"💰 EXIT P&L: ₹{profit:.2f} (Buy @ ₹{avg_price}, Sell @ ₹{sell_price}, Qty={sold_qty})")
                                if partial_booked:
                                    partial_profit = (sell_price - avg_price) * partial_qty
                                    print(f"📊 Total estimated P&L: ₹{(profit + partial_profit):.2f} (exit + partial)")
                                send_telegram(f"💰 EXIT P&L: ₹{profit:.2f}")
                                play_sound_async(SOUND_PROFIT if profit > 0 else SOUND_SL)
                                log_trade_to_excel(instrument.get("internal_trading_symbol"), avg_price, sell_price, sold_qty, profit)
                                display_account_summary(access_token)
                            else:
                                print("⚠️ Could not get executed SELL price. Logging with LTP.")
                                profit = (current_ltp - avg_price) * sell_qty
                                play_sound_async(SOUND_PROFIT if profit > 0 else SOUND_SL)
                                log_trade_to_excel(instrument.get("internal_trading_symbol"), avg_price, current_ltp, sell_qty, profit)
                                display_account_summary(access_token)
                        else:
                        ...
                        profit = (current_ltp - avg_price) * sell_qty
                        print(f"⚠️ Testing mode: Estimated exit P&L ₹{profit:.2f}")
                        play_sound_async(SOUND_PROFIT if profit > 0 else SOUND_SL)
                        log_trade_to_excel(instrument.get("internal_trading_symbol"), avg_price, current_ltp, sell_qty, profit)
                        display_account_summary(access_token)
                    else:
...
def directional_mode():
    ...
    while True:
        ...
        try:
            ...
            # ── ENHANCED: Entry filters ──
            d_filter_passed, d_filter_reasons, d_entry_techs = check_entry_filters(selected_option, option_type, ltp)
            d_oi_passed, d_pcr = check_oi_pcr(cfg["index"], cfg["expiry"], option_type, access_token)
            if not d_oi_passed:
                d_filter_passed = False
                d_filter_reasons.append(f"OI/PCR unfavourable for {option_type} (PCR={d_pcr:.2f})")

            if not d_filter_passed:
                if CONFIG.get("ENTRY_FILTER_STRICT", False):
                    print(f"🚫 Trade BLOCKED by entry filters.")
                    continue
                else:
            ...
            try:
                ...
                # ── ENHANCED: monitoring setup using d_entry_techs ──
                trail_start_time = datetime.now()
                d_atr = cfg["HARD_SL_POINTS"]
                d_entry_vwap = None
                if d_entry_techs:
                    if d_entry_techs.get("atr"):
                        d_atr = d_entry_techs["atr"]
                    d_entry_vwap = d_entry_techs.get("vwap")

                if d_atr and d_atr > 0:
                    hard_sl = round(avg_price - (1.5 * d_atr), 2)
                else:
                    hard_sl = round(avg_price - cfg["HARD_SL_POINTS"], 2)

                trail_step = compute_dynamic_trail_step(d_atr)
                trail_start = cfg["TRAIL_START_PROFIT"]
                poll = cfg["POLL_INTERVAL"]
                max_time = cfg["MAX_TRAIL_TIME"]

                d_partial_booked = False
                d_remaining_qty = bought_qty
                d_partial_qty = bought_qty // 2
                d_partial_threshold = avg_price + cfg.get("PARTIAL_BOOKING_ATR_MULT", 1.5) * (d_atr if d_atr > 0 else cfg["HARD_SL_POINTS"])
                ...
                last_trail_exit = None
                d_ltp_fail = 0
                d_current_ltp = avg_price

                print(f"\n[{trail_start_time.strftime('%H:%M:%S.%f')[:-3]}] 📈 Trailing | SL: ₹{hard_sl:.2f} | Step: {trail_step:.2f} | ATR: {d_atr:.2f}")
                print(f"[{trail_start_time.strftime('%H:%M:%S.%f')[:-3]}] 📊 Partial @ ₹{d_partial_threshold:.2f} | VWAP exit: {'₹'+str(round(d_entry_vwap,2)) if d_entry_vwap else 'N/A'}")
                send_telegram(f"📈 Trailing | Entry: ₹{avg_price:.2f} | SL: ₹{hard_sl:.2f}")

                while True:
                    if time.time() - last_heartbeat > 30:
                        print(f"💓 Monitoring... LTP: ₹{d_current_ltp:.2f} | High: ₹{highest_price:.2f} | Remaining: {d_remaining_qty}")
                        last_heartbeat = time.time()
                    ...
                    # Partial booking
                    if not d_partial_booked and CONFIG.get("ENABLE_PARTIAL_BOOKING", True):
                        if ltp >= d_partial_threshold and d_partial_qty > 0:
                            try:
                                place_market_order_groww(selected_option, d_partial_qty, "SELL", "MIS")
                                d_partial_booked = True
                                d_remaining_qty = bought_qty - d_partial_qty
                                d_partial_profit = (ltp - avg_price) * d_partial_qty
                                print(f"[{datetime.now().strftime('%H:%M:%S.%f')[:-3]}] 📊 PARTIAL BOOK {d_partial_qty}u @ ₹{ltp:.2f} | Locked ₹{d_partial_profit:.2f} | Remaining: {d_remaining_qty}")
                                send_telegram(f"📊 PARTIAL BOOK {d_partial_qty}u @ ₹{ltp:.2f} | Locked ₹{d_partial_profit:.2f}")
                                trail_step = max(cfg.get("TRAIL_STEP_MIN", 0.50), trail_step * 0.7)
                            except Exception as pb_e:
                                print(f"⚠️ Partial booking failed: {pb_e}")

                    if sell_reason is None and CONFIG.get("ENABLE_VWAP_EXIT", True) and d_entry_vwap and d_entry_vwap > 0:
                        if ltp < d_entry_vwap and ltp > avg_price:
                            sell_reason = f"📉 VWAP EXIT: LTP ₹{ltp:.2f} < VWAP ₹{d_entry_vwap:.2f}"
                    ...
                    if sell_reason is None:
                        if ltp <= hard_sl:
                            ...
                            if ltp > highest_price:
                                highest_price = ltp
                                print(f"[{datetime.now().strftime('%H:%M:%S.%f')[:-3]}] 🔼 New High: ₹{highest_price:.2f}")

                            if highest_price >= avg_price + trail_start:
                                trail_exit = round_to_nearest_5_paise(highest_price - trail_step)
                                if trail_exit != last_trail_exit:
                                    print(f"[{datetime.now().strftime('%H:%M:%S.%f')[:-3]}] 📉 Trail | LTP=₹{ltp:.2f} | High=₹{highest_price:.2f} | Exit=₹{trail_exit:.2f} | Step={trail_step:.2f}")
                                    last_trail_exit = trail_exit
                                if ltp <= trail_exit:
                                    sell_reason = f"🔻 Trailing HIT @ ₹{ltp:.2f}"
                    ...
                    if sell_reason:
                        ...
                        play_sound_async(SOUND_SL if ("SL HIT" in sell_reason or "DYNAMIC SL" in sell_reason or "feed lost" in sell_reason) else SOUND_PROFIT)

                        try:
                            sell_order_start = datetime.now()
                            print(f"[{sell_order_start.strftime('%H:%M:%S.%f')[:-3]}] 🔄 Placing SELL order for {d_remaining_qty} units...")
                            sell_order_resp = place_market_order_groww(selected_option, d_remaining_qty, "SELL", "MIS")
                            ...
                            if cfg.get("VALIDATE_ORDERS", True) and sell_order_id:
                                ...
                                sell_status = wait_for_order_status(sell_order_id, access_token, "SELL")
                                if sell_status in ["EXECUTED", "COMPLETED", "DELIVERY_AWAITED"]:
                                    sell_price, sold_qty = get_order_executed_price(sell_order_id, access_token, expected_qty=d_remaining_qty)
                                    if sell_price and sold_qty:
                                        sell_executed = datetime.now()
                                        sell_validation_duration = (sell_executed - sell_validation_start).total_seconds()
                                        profit = (sell_price - avg_price) * sold_qty
                                        complete_trade_duration = (sell_executed - trail_start_time).total_seconds()
                                        print(f"[{sell_executed.strftime('%H:%M:%S.%f')[:-3]}] 💰 EXIT P&L: ₹{profit:.2f} (Buy @ ₹{avg_price}, Sell @ ₹{sell_price})")
                                        if d_partial_booked:
                                            d_partial_profit_est = (sell_price - avg_price) * d_partial_qty
                                            print(f"[{sell_executed.strftime('%H:%M:%S.%f')[:-3]}] 📊 Total est. P&L: ₹{(profit + d_partial_profit_est):.2f}")
                                        print(f"[{sell_executed.strftime('%H:%M:%S.%f')[:-3]}] ⏱️  Validation: {sell_validation_duration:.2f}s | Trade: {complete_trade_duration:.2f}s")
                                        send_telegram(f"💰 EXIT P&L: ₹{profit:.2f}")
                                        play_sound_async(SOUND_PROFIT if profit > 0 else SOUND_SL)
